...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { error: versionError } = await db.from("flow_versions").insert({ flow_id: flow.id, version_number: 1, input_definition: body.inputs, sql: body.sql, output_definition: body.output, duckdb_version: String(body.duckdbVersion ?? "1.32.0") });
