...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: flow, error } = await db.from("flows").select("id,public_id,name,description,categories,visibility,current_published_version,updated_at").eq("public_id", publicId).eq("status", "published").maybeSingle();
