...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: version, error: versionError } = await db.from("flow_versions").select("version_number,input_definition,sql,output_definition,duckdb_version").eq("flow_id", flow.id).eq("version_number", flow.current_published_version).not("published_at", "is", null).single();
    if (versionError) throw versionError;
    return json(request, { publicId: flow.public_id, name: flow.name, description: flow.description, version: version.version_number, inputs: version.input_definition, sql: version.sql, output: version.output_definition, duckdbVersion: version.duckdb_version }, 200);
