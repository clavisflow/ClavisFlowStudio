...
export function assertSafeSql(sql: unknown): asserts sql is string {
  ...
  for (let i = 0; i < sql.length; i++) {
    ...
    if (c === "'") { flush(); state = "single"; content = true; lastSignificantWord = undefined; continue; }
    if (c === '"') { flush(); state = "double"; content = true; lastSignificantWord = undefined; continue; }
    ...
    if (c === ")") { depth--; lastSignificantWord = undefined; }
    if (c === ";" && content) { statements++; content = false; lastSignificantWord = undefined; }
    else if (!/\s/.test(c)) {
      content = true;
      if (c !== "(") lastSignificantWord = undefined;
    }
  }
  flush();
  if (content) statements++;
  const first = words[0];
  const denied = words.find((word, index) =>
    blocked.has(word) && !(word === "REPLACE" && functionCalls.has(index)) ||
    readers.has(word) && functionCalls.has(index));
